  ...
  try {
    ...
    if (document.fonts && document.fonts.load) {
      try {
        ...
        // Ignore. Still render with resolved font.
      }
    }
    return resolved;
  } catch (error) {
...
class Title {
  ...
  createMesh() {
    const { texture, width, height } = createTextTexture(this.gl, this.text, this.font, this.textColor) as any;
...
class App {
  ...
  raf?: number;
  boundOnResize?: () => void;
  boundOnWheel?: ((event: WheelEvent) => void) | undefined;
  boundOnTouchDown?: ((event: any) => void) | undefined;
  boundOnTouchMove?: ((event: any) => void) | undefined;
  ...
  createMedias(items: Array<{ image: string; text: string }> | undefined, bend = 1, textColor: string, borderRadius: number, font: string) {
    const defaultItems = [
  ...
  onTouchMove(e: any) {
    ...
    const position = this.scroll.position ?? this.scroll.current;
    this.scroll.target = position + distance;
  }
